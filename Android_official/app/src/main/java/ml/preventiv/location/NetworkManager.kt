        ...
        firestore.get("hashes").addOnCompleteListener { it ->
            for (documents in it.result!!)
            {
                ...
                for (data in documents.data)
                {
                    ...
                    val amInfectedThroughOnline = (appPreference.isOnlineConnection(uuid))
                    val meNegative = (appPreference.getConfidence() == -1F || appPreference.getConfidence() == 0.25F)
                    if(appPreference.isConnection(uuid) && !amInfected && meNegative)
                    {
                        ...
                        if (timeOfInteraction != (-1).toLong() && timeOfInteraction > firebaseTime && !amInfectedThroughOnline)//found time of interaction & is Greater than Firebase Time
                        {
                            if(key.toInt() < 2)
                            {
